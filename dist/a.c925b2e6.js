...
})({"kZPzW":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "2f43dc47c925b2e6";
"use strict";
...
},{}],"a51Rz":[function(require,module,exports) {
//
//
// Vinh
//
//
const { onAfterLoadPage, onStartLoadPage, onLoadPageError } = require("e8340762cbf2d56e");
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Hàm để load layout
    function loadLayout() {
        return fetch(pageLAYOUT).then((response)=>response.text()).then((html)=>{
            document.body.innerHTML = html;
        });
    }
    // Hàm để load nội dung trang
    async function loadContent(page1) {
        startLoadPage();
        try {
            let res = await fetch(page1);
    ...
    let currentPage = [
        ...pageINDEXs
    ][0]; // default is BASE_DOMAIN
    // Xác định trang cần load dựa trên URL
    pages.forEach((page1)=>{
        if (page1.endsWith(currentPath)) currentPage = page1;
    });
    [
        ...pageINDEXs
    ].forEach((page1)=>{
        if (page1.endsWith(currentPage)) currentPage = pageINDEXs[1];
    });
    ...
    pages.forEach((pageLINK)=>{
        ...
        page(routePage, function() {
            ...
            if ([
                ...pageINDEXs
            ].includes(currentRoutePage)) contentPAGE = "index";
...
},{"e8340762cbf2d56e":"4NxvW"}]},["kZPzW","a51Rz"], "a51Rz", "parcelRequirea605")
